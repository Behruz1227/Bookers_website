...
export const MasterClassModal = () => {
  ...
  const [status, setStatus] = useState<
    "MasterClass" | "OTPcode" | "Ok" | "Error"
  >("MasterClass");
  const [phoneNumber, setPhoneNumber] = useState<string>("+998");
  const [OTPcode, setOTPcode] = useState<null | string>(null);
  const [btnStatus, setBtnStatus] = useState<boolean>(false);
  const [selectedHour, setSelectedHour] = useState<null | number | string>(
    null
  ); // Boshlang'ich holat null
  const [selectedMinute, setSelectedMinute] = useState<null | number | string>(
    null
  ); // Boshlang'ich holat null
  ...
  useEffect(() => {
    if (SendCode && masterClassHolat) {
      ...
      CheckCode?.body === formData.contactInformation &&
  ...
  return (
    <div>
      ...
      >
        <div className="w-full grid place-items-center my-3 px-[2%] mb-10">
          ...
          {status === "MasterClass" && (
            <div className="grid grid-cols-1 lg:grid-cols-2 justify-center  gap-y-2 lg:gap-3 w-full ">
              <div className="col-start-1 col-end-3 lg:col-end-2">
                <label
                  className="block truncate text-gray-700 font-medium  mb-2"
                  htmlFor="firstName"
                >
                  {t("Имя мастера или название салона*")}
                </label>
                <input
                  type="text"
                  id="firstName"
                  ...
                  className={`border-2 ${
                    errors.nameSalonOrMaster
                      ? "border-red-500"
                      : "border-gray-700"
                  } bg-[#B9B9C9]  p-3 rounded-xl w-full focus:outline-none focus:ring-0 `}
                />
              </div>
              <div className="col-start-1 lg:col-start-2 col-end-3">
                <label
                ...
                >
                  ...
                  <option value="OTHER">
                    {t("Другое обучающее мероприятие")}
                  </option>
                </select>
              </div>
              <div className="col-start-1 col-end-3">
                ...
                <textarea
                  ...
                  onChange={handleChange}
                  className={`border-2 ${
                    errors.eventName ? "border-red-500" : "border-gray-700"
                  } bg-[#B9B9C9]  p-3 rounded-xl w-full focus:outline-none focus:ring-0 `}
                />
              </div>
              <div className="col-start-1 col-end-3 lg:col-end-2">
                        ...
                        <div className="grid grid-cols-2 justify-center p-[5%] gap-x-[5%]">
                          ...
                          <div className="border-none">
                            {hours.map((hour) => (
                              <button
                                key={hour}
                                className={`w-full px-4  py-2 text-left text-sm rounded-[10px] hover:text-white ${
                                  selectedHour === hour
              ...
              <div className="col-start-1 col-end-3 lg:col-end-2">
                <label
                  className="block text-gray-700 font-medium truncate mb-2"
                  htmlFor="lastName"
                >
              ...
              <div className="col-start-1 lg:col-start-2 col-end-3">
                ...
                <input
                  ...
                  onChange={handleChange}
                  className={`border-2 ${
                    errors.eventLocation ? "border-red-500" : "border-gray-700"
                  } bg-[#B9B9C9]  p-3 rounded-xl w-full focus:outline-none focus:ring-0 `}
                />
              </div>
              <div className="col-start-1 col-end-3 lg:col-end-2">
          ...
          {status === "OTPcode" && (
            <div className="w-full grid place-items-center">
              <p className="font-bold text-2xl my-3">{phoneNumber}</p>
              <p className="text-[#59595c] text-center">
                {t("Мы отправили вам SMS с кодом подтверждения.")}
              </p>
              <div className="w-[90%] lg:w-[50%] otp-input p-6">
                <Input.OTP
                  length={4}
                  onInput={(value) => {
          ...
          {status === "Error" && (
            <div className="grid place-items-center">
              ...
              <p className="text-xl w-[100%] mt-6 text-slate-600 text-center">
                {t(
                  "Что бы оставить заявку на мастеркласс  необходимо пройти регистрацию мастера"
                )}
              </p>
            </div>
          )}
          {status === "Ok" && (
            <div className="grid place-items-center">
              ...
              <p className="text-xl w-[100%] mt-4 text-slate-600 text-center">
                {t(
                  "Ваша заявка принята и скоро будет опубликована на этом сайте в разделе “НОВОСТИ”"
                )}
              </p>
            </div>
          )}
          <div className="mt-1">
            <button
              onClick={() => {
                if (status === "MasterClass") {
                  ...
                  if (OTPcode?.length === 4) {
                    ...
                    toastBtn(
                      t("Введите действительный одноразовый пароль"),
                      "error"
                    );
                  }
              ...
              className={`mt-4  py-2 md:py-3 px-8 md:px-10  rounded-full 
                                ${status !== "Ok" ? "" : "hidden"} 
                                ${
                                  !btnStatus
                                    ? "bg-[#9C0B35] text-white"
                                    : "bg-[#d12253] text-white0"
                                }`}
            >
              {status === "OTPcode" &&
              ...
              {status === "MasterClass" &&
                (btnStatus ? t("Загрузка") : t("Отправить заявку"))}
              {status === "Error" && t("Зарегистрироваться")}
            </button>
          </div>
        </div>
      </UniversalModal>
