...
import {
  leaveFeedbackMasterOrSalonSearch,
  leaveFeedbackSave,
} from "@/helpers/Url";
import { useTranslation } from "react-i18next";

export const LeaveFeedback = () => {
  const { setOtzivHolat, otzivHolat, setLoginRole, setLoginHolat } =
    LoginIndex();
  const [status, setStatus] = useState<"otziv" | "OTPcode" | "Ok" | "Error">(
    "otziv"
  );
  ...
  const [masterOrSalonStatus, setMasterOrSalonStatus] =
  ...
  const { response: res, globalDataFunc: globalDataFunc2 } = useGlobalRequest(
    leaveFeedbackSave,
    "POST",
    data
  );
  ...
  const options = response?.body.map((item: MasterOrSalon) => ({
    ...
    label: (
      <span className={`${item.roleName ? "text-[#9C0B35]" : "text-blue-700"}`}>
  ...
  useEffect(() => {
    if (
      ...
      errorSendCode == "Request failed with status code 400" &&
      otzivHolat
    ) {
      setStatus("Error");
      setFirstName("");
      ...
      res?.message === "Sizning sharhingiz qabul qilindi. Rahmat!" &&
      res?.success === true &&
      status === "OTPcode" &&
      otzivHolat
    ) {
  ...
  return (
    <>
      ...
      <UniversalModal
        ...
        style=""
      >
        <div className="w-full grid place-items-center my-2 mb-10">
          {status === "otziv" && (
            <div className="">
              <h1 className="font-bold text-xl lg:text-xl px-[2%] text-center">
                {t(
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 justify-center  gap-y-4 lg:gap-6 w-full my-6 px-[2%]">
                ...
                <div className="col-start-1 lg:col-start-2 col-end-2 lg:col-end-3">
                  <label
                    ...
                    htmlFor="phone"
                  >
                    {t("Название мастера или салона красоты*")}
                  </label>
                  <Select
                    showSearch
                    allowClear
                    ...
                    className="border-2 masterOrSalonName border-gray-700 bg-[#B9B9C9] py-0 px-6 rounded-xl w-full focus:outline-none focus:ring-0"
                    options={options}
                    ...
                    onChange={(value: string) => {
                      const selectedOption = options.find(
                        (option: { value: string; roleName: string | null }) =>
                          option.value === value
                      );
                      setMasterOrSalon(selectedOption?.value);
                      setMasterOrSalonStatus(
                        selectedOption?.roleName == null ? false : true
                      );
                    }}
                    onClear={() => {
                      setMasterOrSalon("");
              ...
              <div className="text-center flex justify-center ">
                <p className="text-slate-600 my-4">
                  ...
                  {t(
                    "Я согласен(на) на публикацию моего отзыва на сайте и в мобильном приложении bookers."
                  )}
                </p>
              </div>
            </div>
          ...
          {status === "OTPcode" && (
            <div className="w-full grid place-items-center">
              <h1 className="font-bold text-3xl">{t("ОТП код")}</h1>
              <p className="font-bold text-2xl my-3">{phone}</p>
              ...
              {timer !== null && timer > 0 ? (
                <p className="text-[#59595c]">
                  {t("Отправить код заново")}
                  {timer} {t("сек")}
                </p>
              ) : (
                <p
                  className="text-[#9C0B35]"
                  onClick={() => {
                    handleResendCode();
                    SendCodeBtn();
                  }}
          ...
          {status === "Ok" && (
            <div className="grid place-items-center">
              ...
              <p className="text-center text-xl">
                {t("Спасибо что помогаете улучшить наш сервис")}
              </p>
            </div>
          )}
          {status === "otziv" && (
